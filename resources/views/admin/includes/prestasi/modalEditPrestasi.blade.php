<!-- Modal Edit Prestasi -->
<div class="modal fade" id="modalEditPrestasi-{{ $item->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
                ...
                <div class="row g-3">
                    <form class="row g-3" action="/update_prestasi/{{ $item->id }}" method="POST">
                        ...
                        <div class="col-md-6 mb-1">
                            ...
                            <input disabled readonly type="text" name="nim"
                                class="form-control @error('nim') is-invalid @enderror"
                                value="{{ old('nim', $item->nim) }}">
                            <input hidden type="text" name="nim"
                                class="form-control @error('nim') is-invalid @enderror"
                                value="{{ old('nim', $item->nim) }}">
                            @error('nim')
                                <div class="invalid-feedback">
                        ...
                        <div class="input-group col-12 mb-1">
                            ...
                            <select disabled name="jurusan" class="form-control @error('jurusan') is-invalid @enderror">
                                <option value="{{ old('jurusan', $item->jurusan) }}">
                                    {{ old('jurusan', $item->jurusan) }}</option>
                                <option value="Teknik Informatika" @if (old('jurusan') == 'Teknik Informatika') selected @endif>
                        ...
                        <div class="col-12 mb-1">
                            ...
                            <input type="text" name="lomba"
                                class="form-control  @error('lomba') is-invalid @enderror"
                                value="{{ old('lomba', $item->lomba) }}">
                            @error('lomba')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="input-group col-8 mb-1">
                            ...
                            <select class="form-control @error('tingkat') is-invalid @enderror" name="tingkat">
                                <option value="{{ old('tingkat', $item->tingkat) }}">
                                    {{ old('tingkat', $item->tingkat) }}</option>
